...
# Make decesions on whether to keep vars or not

attach(states)
murder_model <- lm(Murder ~ 
                     Illiteracy +
                     Population +
                     HS_Grad +
                     Income +
...
library(car)
qqPlot(murder_model, labels = row.names(states),
       id.method = "Identity",
       simulated = TRUE,
       main = "Q-Q Plot")
...
curve(dnorm(x, mean = mean(student_murder_model),
            sd = sd(student_murder_model)), add = TRUE,
            col = "blue", lwd = 2)
...
# Remove Nevada from the data set
states <- subset(states, states$Area)